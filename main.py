...
LAST_NOTIF_DATE=""; LAST_PAGI_DATE=""
def save_chat_id(cid):
    try:
        CHAT_IDS.add(cid)
        open(CHAT_FILE,"w").write(",".join(map(str,CHAT_IDS)))
    except: pass
def load_chat_ids():
    try:
        if os.path.exists(CHAT_FILE):
            for x in open(CHAT_FILE).read().split(","):
...
def self_ping():
    import requests
    while True:
        try: time.sleep(240); requests.get("http://127.0.0.1:8080/ping", timeout=5)
        except: pass
...
def bulet_idx(price):
    t=1 if price<200 else 2 if price<500 else 5 if price<2000 else 10 if price<5000 else 25
    ...
    for sym in [symbol, symbol+".JK", symbol.upper()+".JK"]:
        try:
            ...
            if df is not None and not df.empty and len(df)>20:
                df=flatten_df(df).dropna(subset=['Close'])
...
def predict_next(df):
    try:
        ...
        e5p=float(ema5.iloc[-2]); e10p=float(ema10.iloc[-2]); e5_2=float(ema5.iloc[-3])
        ...
        curr_close=float(close.iloc[-1]); prev_close=float(close.iloc[-2])
        score=50; reasons=[]
        ...
        if curr_close>prev_close and prev_close>float(close.iloc[-3]): score+=5; reasons.append(f"Naik 2 hari berturut +5%")
...
def generate_chart_fixed(df,symbol,mode="PASTI"):
    ...
    swing_entry=bulet_idx(curr_close); swing_sl=bulet_idx(curr_close*0.96); swing_tp1=bulet_idx(curr_close*1.07); swing_tp2=bulet_idx(curr_close*1.12); swing_tp3=bulet_idx(curr_close*1.20)
    ...
    plot_df['BuySignal']=df['BuySignal'].tail(100); plot_df['SellSignal']=df['SellSignal'].tail(100)
    ...
    ax_price.plot(plot_df.index,plot_df['EMA20'],label='EMA20',color='purple',linewidth=1)
    for idx,row in plot_df.iterrows():
        try:
            if row['BuySell']=='BUY': ax_price.annotate('BUY',xy=(idx,float(row['Low'])*0.985),bbox=dict(boxstyle="round,pad=0.4",fc="#089981",ec="#089981",alpha=0.9),color='white',fontsize=9,weight='bold',ha='center')
            elif row['BuySell']=='SELL': ax_price.annotate('SELL',xy=(idx,float(row['High'])*1.015),bbox=dict(boxstyle="round,pad=0.4",fc="#F23645",ec="#F23645",alpha=0.9),color='white',fontsize=9,weight='bold',ha='center')
        except: pass
    last=plot_df.iloc[-1]
    trend="BULLISH" if float(plot_df['EMA5'].iloc[-1])>float(plot_df['EMA10'].iloc[-1])>float(plot_df['EMA20'].iloc[-1]) else "BEARISH"
    icon = "🚀" if "NAIK" in pred else "🔻" if "TURUN" in pred else "➡️"
    pasti_tag = "🔥 PASTI" if score>=80 else "⚡ KEMUNGKINAN" if score>=70 else "⚠️"
    data_date = plot_df.index[-1].strftime('%Y-%m-%d'); now_wib = datetime.datetime.now(WIB).strftime('%H:%M WIB %d-%b')
...
def analyze_pasti(symbol, min_price=50):
    try:
        ...
        if not (45 <= rsi <= 80): return None
        close=pd.Series(flatten_df(df.copy())['Close']).dropna()
        if not (calc_ema(close,5).iloc[-1]>calc_ema(close,10).iloc[-1]): return None
        if curr_close < calc_ema(close,5).iloc[-1]: return None
...
def auto_notif_loop():
    ...
    while True:
        try:
            now=datetime.datetime.now(WIB); today_str=now.strftime("%Y-%m-%d")
            if now.hour==9 and now.minute in [15,16] and LAST_PAGI_DATE!=today_str and len(CHAT_IDS)>0:
                ...
                txt=f"🔥 AUTO PASTI 09:15 - {today_str}\nHanya yang 80%+ & Vol rame!\n\n" if results else f"🔔 AUTO PASTI 09:15 - {today_str}\nGak ada yang PASTI 80%+"
                for r in results: txt+=f"✅ {r['symbol']} {r['close']:.0f} {r['score']}% Vol {r['vol']:.1f}x RSI {r['rsi']:.0f} ({r['date']})\n  /pasti {r['symbol'].lower()}.jk\n"
                if results: txt+="\nYang PASTI aja!"
            ...
            if now.hour==15 and now.minute in [30,31,35] and LAST_NOTIF_DATE!=today_str and len(CHAT_IDS)>0:
                results=[r for r in [analyze_pasti(s) for s in WATCHLIST[:60]] if r]
                results=sorted(results,key=lambda x:x['score'],reverse=True)[:5]
                txt=f"🔥 AUTO PASTI SORE 15:30 - {today_str}\n" if results else f"🔔 AUTO SORE 15:30 - {today_str}\nGak ada yang PASTI"
                for r in results: txt+=f"✅ {r['symbol']} {r['close']:.0f} {r['score']}%\n  /sore {r['symbol'].lower()}.jk\n"
                for cid in list(CHAT_IDS):
                    try: bot.send_message(cid, txt)
                    except: pass
                LAST_NOTIF_DATE=today_str; time.sleep(120)
            time.sleep(30)
...
def process_stock_request(message, mode="PASTI"):
    save_chat_id(message.chat.id); args=message.text.split()[1:]
    if not args: bot.reply_to(message,f"Gunakan: /{mode.lower()} BRMS.JK"); return
    sym=args[0].upper().replace(".JK.JK",".JK"); loading=bot.reply_to(message,f"⏳ {mode} {sym} cek REALTIME...")
...
def handle_modes(message):
    process_stock_request(message, message.text.split()[0].replace('/','').upper())
...
def handle_scan(message):
    ...
    if pasti_mode:
        loading=bot.reply_to(message,f"🔍 V15 REALTIME Scanning 60 saham 80%+...")
        try:
            results=[r for r in [analyze_pasti(s) for s in WATCHLIST[:60]] if r]
        ...
        loading=bot.reply_to(message,f"🔍 V15 Scanning 60 saham...")
        try:
            results=[]
            for sym in WATCHLIST[:60]:
                df,_=get_data_realtime(sym)
                if df is None: continue
                pred,score,reasons,vol,rsi,close=predict_next(df)
                if score>=70 and "NAIK" in pred: results.append({'symbol':sym.replace('.JK',''), 'close':close, 'score':score, 'vol':vol, 'date': df.index[-1].strftime('%Y-%m-%d')})
            results=sorted(results,key=lambda x:x['score'],reverse=True)
            txt=f"🔥 V15 SCAN - {datetime.datetime.now(WIB).strftime('%d %b')}\nTotal {len(results)}\n\n"
            for i,r in enumerate(results[:15],1): txt+=f"{i}. {r['symbol']} {r['close']:.0f} {r['score']}% {r['date']}\n"
            bot.reply_to(message,txt); bot.delete_message(loading.chat.id,loading.message_id)
        except Exception as e: bot.edit_message_text(f"Error: {e}"[:400],loading.chat.id,loading.message_id)
