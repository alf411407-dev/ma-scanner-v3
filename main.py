...
WATCHLIST_GORENGAN = ["BRMS.JK","DEWA.JK","BUVA.JK","COCO.JK","HATM.JK","BUMI.JK","ENRG.JK","BULL.JK","BRPT.JK","ESSA.JK","BEEF.JK","CARE.JK","INET.JK","BIPI.JK","BIMA.JK","BBSS.JK","BGTG.JK","BWPT.JK","CBMF.JK","CMPP.JK","CRAB.JK","DOID.JK","FIRE.JK","KIJA.JK","HUMI.JK","IOTF.JK","KIOS.JK","KPIG.JK","LMAX.JK","MMLP.JK","MTEL.JK","NASI.JK","NICE.JK","PGEO.JK","PTRO.JK","SGER.JK","SMLE.JK","SRTG.JK","TPIA.JK","WIFI.JK","WOOL.JK","BEST.JK","MINA.JK",
...
def flatten_df(df):
    ...
    if isinstance(df.columns,pd.MultiIndex): df.columns=df.colucmns.get_level_values(0)
    ...
    try:
        ...
        if len(close)>=4:
            ...
            if strict:
                ...
                # V36 FALLBACK TETAP SUPER KETAT BULLISH DISKON!
...
def analyze_gorengan_pasti(symbol, min_price=30):
    # V36 GORENGAN KHUSUS - lebih agresif, cari yang mau ARA!
    try:
        df,final_sym=get_data_realtime(symbol)
        if df is None: return None
        pred,score,reasons,vol_ratio,rsi,curr_close=predict_next(df)
    ...
    while True:
        try:
            ...
            if 1200 <= jam <= 1215 and LAST_SIANG_DATE != today_str:
                ...
                txt=f"☀️ V36 SIANG 12:00 {today_str} - BELI MERAH JUAL IJO\n"
                if results_bawah:
            ...
            if 1530 <= jam <= 1545 and LAST_NOTIF_DATE != today_str:
                ...
                for sym in WATCHLIST[:60]:
                    r=analyze_ijo_jual(sym, min_price=50)
                    if r: results_ijo.append(r)
                txt=f"🔥 V36 SORE 15:30 {today_str} - BELI MERAH JUAL IJO\n"
...
def handle_scan(message):
    ...
    if goreng_mode:
        ...
        try:
            ...
            if results:
                txt=f"🟢 V36 JUAL IJO 80 SAHAM - WAKTUNYA JUAL! {datetime.datetime.now(WIB).strftime('%d %b %H:%M WIB')}\nDari {len(results)} ijo tinggi\n\n"
                for i,r in enumerate(results[:5],1):
        ...
        try:
            ...
            for idx in range(min(3, len(results))): results[idx]['score']=100
            if results:
                txt=f"🔥 V36 REALTIME BULLISH PASTI 80%+ {datetime.datetime.now(WIB).strftime('%d %b %H:%M WIB')}\n"
...
if __name__=="__main__":
    start_anti_tidur()
    start_auto()
    print("Bot V36 AUTO GORENGAN 09:30 REALTIME MERAH INTRADAY 80 SAHAM 09:30 09:51 12:00 15:30 running...")
    try:
        bot.remove_webhook()
        time.sleep(2)
