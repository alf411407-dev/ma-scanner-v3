...
def home():
    ...
    return f"Bot V20 ULTRA BAWAH - Uptime {uptime//3600}h {(uptime%3600)//60}m {datetime.datetime.now(WIB).strftime('%H:%M:%S WIB')}"
@app.route('/health')
...
def handle_scan(message):
    ...
    if bawah_mode:
        loading=bot.reply_to(message,f"🔍 V20 ULTRA BAWAH Scanning >{min_price} Pump<10%...")
        try:
            results=[]
            for sym in WATCHLIST[:60]:
            ...
            if results:
                ...
                txt=f"🔍 V20 ULTRA BAWAH >{min_price} - Gak ada ULTRA BAWAH hari ini, filter ketat!"
